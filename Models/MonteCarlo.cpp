...
double MonteCarlo::calculatePrice_par(double spot, unsigned unifStartSeed, int numScenarios) {
    bool barrierHit = (barrierType == BarrierType::up_and_out && spot >= barrierValue)||
        (barrierType == BarrierType::down_and_out && spot <= barrierValue);
    ...
    if (option.timeToExpiry() > 0.0) {
        ...
        vector<std::future<vector<double>>> futures;
        futures.reserve(numScenarios);
        ...
        for (int i=0; i<numScenarios; ++i) {
            ...
            futures.push_back(std::async(gen, unifDist(mtUnif)));

        }
        for (auto& future : futures){
            // unifDist(mtUnif) provides the next seed):
            vector scenario = future.get();
